...
coeffs = logisticReg.coef_  # Shape: (n_classes, n_features)

# Calculate the absolute importance for each class and overall
classImportances = np.abs(coeffs)  # Shape: (n_classes, n_features)
overallImportance = np.mean(classImportances, axis=0)

# Normalize to obtain relative importance for each class and overall
normClassImportances = classImportances / np.sum(classImportances, axis=1, keepdims=True)
normOverallImportance = overallImportance / np.sum(overallImportance)

# Threshold the relative importances to show only those with greatest impact
pieThreshold = 0.023
metabolite_importances = [value for value in normOverallImportance if value > pieThreshold]
metabolite_inds = [i for i, value in enumerate(normOverallImportance) if value > pieThreshold]

# Determine the relative importance of 'other' and how many
other_importance = sum(normOverallImportance) - sum(metabolite_importances)
nOthers = sum(1 for score in normOverallImportance if score < pieThreshold)
...
plt.title(f'Logistic Regression Metabolite Importances')
plt.show()

# Generate pie plots for each class
for class_idx in range(yTrain.shape[1]):
    
    # Extract the importances for the current class
    normImportance_class = normClassImportances[class_idx,:]
    
    # Apply threshold
    metabolite_importances_class = [value for value in normImportance_class if value > pieThreshold]
    metabolite_inds_class = [i for i, value in enumerate(normImportance_class) if value > pieThreshold]
    
    # Compute 'other' importance and how many
    other_importance_class = 1.0 - sum(metabolite_importances_class)
    nOthers_class = sum(1 for score in normImportance_class if score < pieThreshold)
    ...
    plt.title(f'Logistic Regression Metabolite Importances for {subjectLabels[class_idx]} Group')
    plt.show()